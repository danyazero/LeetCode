...
import com.danyazero.executorservice.utils.JavaCodeExecutor;
...
public class Test {
    ...
    public String test(@RequestBody SchemaDto schemaDto) {
        if (schemaDto.language().equals("java")) {
            var codeExecutor = new JavaCodeExecutor();
            return codeExecutor.apply(MethodSchema.parse(schemaDto.schema()), schemaDto.solution());
        }
